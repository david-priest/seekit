plotBoxplotsProportions <- function(
    sce,
    ...
    # ---- NPC (Prism-style brackets, 2D facet: master x daughter) ----
  ...
  # Set factor levels for master_cluster_column and daughter_cluster_column
  ...
  df[[group_column]] <- factor(df[[group_column]], levels = if (!is.null(group_order)) group_order else unique(df[[group_column]]))
  df[[daughter_cluster_column]] <- factor(df[[daughter_cluster_column]], levels = if (!is.null(daughter_order)) daughter_order else unique(df[[daughter_cluster_column]]))
  
  if (is.null(group_palette)) 
    group_palette <- RColorBrewer::brewer.pal(min(length(levels(df[[group_column]])), 12), "Paired")
  
  count_df <- df %>%
    dplyr::group_by_at(dplyr::vars(dplyr::all_of(c(sample_id_column, master_cluster_column, daughter_cluster_column, group_column)))) %>%
    dplyr::summarise(count = dplyr::n(), .groups = "drop")
  
  complete_df <- count_df %>%
    tidyr::complete(
      tidyr::nesting(!!rlang::sym(sample_id_column), !!rlang::sym(master_cluster_column), !!rlang::sym(group_column)), 
      !!rlang::sym(daughter_cluster_column), 
      fill = list(count = 0)
    )
  
  proportion_df <- complete_df %>%
    dplyr::group_by_at(dplyr::vars(dplyr::all_of(c(sample_id_column, master_cluster_column, group_column)))) %>%
    dplyr::mutate(proportion = count / sum(count) * 100) %>%
    dplyr::ungroup()
  
  summary_df <- proportion_df %>%
    dplyr::rename(cell_count = count)

  summary_out <<- summary_df
  
  p <- ggplot2::ggplot(summary_df, ggplot2::aes(x = !!rlang::sym(group_column), y = proportion, fill = !!rlang::sym(group_column))) +
  ...
  if (show_stats) {
    ...
    if (identical(stats_style, "npc")) {
        # external_stats is expected to have columns: master_cluster_column,
        # daughter_cluster_column (typically "isotype"), group1, group2,
        # p.adj.signif, and ideally p.adj. We compute panel_max as max of
        # proportion per (master, daughter) panel.
        ...
        if (is.null(external_stats)) {
            warning("plotBoxplotsProportions: stats_style='npc' currently requires external_stats. Skipping bracket layer.")
            npc_stat <- NULL
        } else {
    ...
    if (!is.null(external_stats)) {
      ...
      external_stats[[master_cluster_column]] <- factor(external_stats[[master_cluster_column]], levels = levels(df[[master_cluster_column]]))
      summary_df[[master_cluster_column]] <- factor(summary_df[[master_cluster_column]], levels = levels(df[[master_cluster_column]]))
      ...
      max_y_df <- summary_df %>%
        dplyr::group_by_at(dplyr::vars(dplyr::all_of(c(master_cluster_column, daughter_cluster_column)))) %>%
        dplyr::summarise(max_val = max(proportion, na.rm = TRUE), .groups = "drop") %>%
        dplyr::mutate(max_val = ifelse(max_val == 0 | is.na(max_val), 1, max_val))
      
      stat.test <- external_stats %>%
        dplyr::left_join(max_y_df, by = c(master_cluster_column, daughter_cluster_column)) %>%
        dplyr::group_by_at(dplyr::vars(dplyr::all_of(c(master_cluster_column, daughter_cluster_column)))) %>%
        dplyr::mutate(
      ...
      stat.test <- summary_df %>%
        dplyr::group_by_at(dplyr::vars(dplyr::all_of(c(master_cluster_column, daughter_cluster_column)))) %>%
        rstatix::dunn_test(formula = as.formula(paste("proportion ~", group_column)), p.adjust.method = "holm") %>%
