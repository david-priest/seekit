plotPbExprsCondMulti <- function (x, k = "meta20", scales = "free_x", conditions = NULL, excluded_clusters = NULL, axes = "all",
                                  features = "state", assay = "exprs", fun = c("median", "mean", "sum"), 
                                  point_size = 1, clusters_order = NULL, textsize = 14, panel_spacing = 2, show_stats = TRUE, hide_ns = TRUE,
                                  facet_by = c("antigen", "cluster_id"), color_by = "condition", mean_or_med = "median",
                                  group_by = color_by, shape_by = NULL, size_by = FALSE, stat_size = 4, facet_ratio = 1.5,
                                  geom = c("boxes","bar"), jitter = TRUE, group1keep = NULL, nudge = 0, step_increase = 0,
                                  hide_x_labels = FALSE) 
{
  ...
  stat.test <- df %>%
    ...
    wilcox_test(as.formula(paste("value ~", color_by)))  # For stats between color_by.. Updated to accept color_by as a paramater.
  ...
  p <- ggplot(df, aes_string(x = group_by, y =  "value")) + 
    facet_grid2(cluster_id ~ antigen, scales = scales, independent = "y", axes = axes) + 
    labs(y = paste(fun, ifelse(assay == "exprs", "expression", assay))) + # Now it just plots "median expression"
    theme_bw() + 
    theme(
